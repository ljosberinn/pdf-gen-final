...
class FPDI extends FPDF_TPL
{
    ...
    public function placeName($name) {
        $this->SetXY(103, 13.75);
        $this->MultiCell(48.25, 4.45, $_SESSION['name']);
    }

    public function placePersonalnummer($personalnummer) {

        $this->SetXY(190.5, 13.75);
        $this->MultiCell(10, 4.45, $_SESSION['personalnummer']);
    }

    public function placeStartEndTime($arr) {

        $start = date('H:i', $arr[0]);
        $end = date('H:i', $arr[1]);

        $this->SetXY(138, 21.5);
        $this->MultiCell(11, 4.45, $start);

        $this->SetXY(173, 21.5);
        $this->MultiCell(11, 4.45, $end);
    }

    public function placeDate($day) {
        $this->SetXY(100, 21.5);
        $this->MultiCell(20, 4.45, date('d.m.Y', $day));
    }

    public function placeKostenstelle($kostenstelle, $yCoord) {
        $kostenstelleSplit = str_split($kostenstelle);

        for ($k = 0; $k <= 2; $k += 1) {
            $xCoord = 22 + $k * 7;
            $this->SetXY($xCoord, $yCoord);
            $this->MultiCell(12.9, 3.5, $kostenstelleSplit[$k]);
        }
    }

    public function placeAuftragsnummer($auftragsnummer, $yCoord) {
        $auftragsnummerSplit = str_split($auftragsnummer);

        for ($k = 0; $k <= 5; $k += 1) {
            $xCoord = 42 + $k * 4.8;
            $this->SetXY($xCoord, $yCoord);
            $this->MultiCell(12.9, 3.5, $auftragsnummerSplit[$k]);
        }
    }

    public function placeKunde($kunde, $yCoord) {
        $this->SetFontSize(8);
        $this->SetXY(71.6, $yCoord);
        $this->MultiCell(53.55, 3.5, $kunde);
        $this->SetFontSize(16);
    }

    public function placeLeistungsart($leistungsart, $yCoord) {
        $leistungsartSplit = str_split($leistungsart);

        for ($k = 0; $k <= 2; $k += 1) {
            $xCoord = 128.318 + $k * 7.5;
            $this->SetXY($xCoord, $yCoord);
            $this->MultiCell(12.9, 3.5, $leistungsartSplit[$k]);
        }
    }

    public function placeMinuten($minuten, $yCoord) {
        $minutenSplit = str_split($minuten);
        $minutenLength = count($minutenSplit);

        switch($minutenLength) {
        case 1:
            $minutenStart = 159.884;
            break;
        case 2:
            $minutenStart = 154.784;
            break;
        case 3:
            $minutenStart = 149.684;
            break;
        }

        for ($k = 0; $k <= ($minutenLength - 1); $k += 1) {
          $xCoord = $minutenStart + $k * 5.1;
            $this->SetXY($xCoord, $yCoord);
            $this->MultiCell(12.9, 3.5, $minutenSplit[$k]);
        }
    }

    public function placeAnzahl($anzahl, $yCoord) {
        $anzahlSplit = str_split($anzahl);
        $anzahlLength = count($anzahlSplit);
        ...
        for ($k = 0; $k <= ($anzahlLength - 1); $k += 1) {
            $xCoord = $anzahlStart + $k * 4.5;
            $this->SetXY($xCoord, $yCoord);
            $this->MultiCell(12.9, 3.5, $anzahlSplit[$k]);
        }
    }

    public function placeMaterialnummer($materialnummer, $yCoord) {
        $materialnummerSplit = str_split($materialnummer);
        $materialnummerLength = count($materialnummerSplit);

        switch($materialnummerLength) {
        case 1:
            $materialnummerStart = 195.866;
            break;
        case 2:
            $materialnummerStart = 191.451;
            break;
        case 3:
            $materialnummerStart = 187.186;
            break;
        case 4:
            $materialnummerStart = 182.754;
            break;
        }

        for ($k = 0; $k <= ($materialnummerLength - 1); $k += 1) {
            $xCoord = $materialnummerStart + $k * 4.37;
            $this->SetXY($xCoord, $yCoord);
            $this->MultiCell(12.9, 3.5, $materialnummerSplit[$k]);
        }
    }
